...
# - Use bagged trees with 10-fold CV using train() function in caret package,
#   to test future performance
library(ipred)
RNGversion("3.5.2")
set.seed(300)
ctrl <- trainControl(method = "cv", number = 10)
...
## Boosting
  ...
  - <b>boosting</b> : it boosts performance of weak learners to attain the performance of stronger learners.
  ...
  - <b>AdaBoost</b> or <b>adaptive boosting</b>, the algorithm is based on the idea of generating weak learners that iteratively learn a larger portion of the difficult-to-classify examples in the training data by paying more attention.
  - The <b>AdaBoost.M1</b> algorithm provides another tree-based implementation of AdaBoost for classification.
...
# - it returns an object with information about the model
...
## Random forests
  ...
  - <b>random forests</b> or <b>decision tree forests</b>: focuses on ensembles of decision trees.  Combines the principles of bagging with random feature selection to add additional diversity to the decision tree models.
    - <b>Strengths</b>:
...
```
  ...
  - This confusion matrix does not show re-substitution error.  Instead, it reflects the <b>out-of-bag error rate</b> (list in the output as OOB estimate of error rate) which, unlike re-substitution error, is an unbiased estimate of the test set error.
    - The out-of-bag estimate is computed during the construction of the random forest.
    - At the end of the forest construction, for each of the 1000 examples in the dataset, the trees that did not use the example in training are allowed to make a prediction.
  - To calculate the kappa statistic on the out-of-bag predictions, we can use the function in the cvd pakage as fllows.
...
### Evaluating random forest performance in a simulated competition
  ...
  - randomForest() is supported by caret, which allows us to optimize the model wwhile at the same time calculating performance measures beyond the out-of-bag error rate.
  
```{r}
# - First we'll set the 'savePredictions' and 'classProbs' parameters to TRUE, 
#   which sames the holdout sample predictions and predicted probabilities for 
#   plotting the ROC curve late.
# - Then, we'll also set the 'summaryFunction' to 'twoClassSummary', which is a 
#   'caret' function that computes performance metrics like AUC.
...
# - Next we'll setup the tuning grid for the random forest.
#   - the only parameter is 'mtry', which defines how many features are randomly 
#     selected at each split.
grid_rf <- expand.grid(mtry = c(2, 4, 8, 16))

# - Supply the resulting grid to the train() function
RNGversion("3.5.2")
set.seed(300)
...
# - Compare the best forest to the best boosted decision tree
# - When the random forests are finished training, we'll compare the best forest
#   to the best boosted decision tree among trees with 10, 25, 50, and 100
#   iterations using the caret package
...
# - Visualize their performance, we use the pROC package
#install.packages("pROC")
library(pROC)
roc_rf <- roc(m_rf$pred$obs, m_rf$pred$yes)
roc_c50 <- roc(m_c50$pred_obs, m_c50$pred$yes)
